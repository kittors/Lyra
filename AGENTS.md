...
### 照着别处的样子做一个东西时，换的是深度，不是圆角

「参考聊天输入框」「和那个卡片一样」——这种需求给的是**视觉语言**：圆角多大、描边多细、内衬
多宽、控件怎么排。把那一串类名整段抄过来不叫参考，叫搬家，而搬过来的东西里总有几样是原地才
成立的。

- **阴影跟着底子走。** 主输入框那两层阴影是它和页面之间的距离；同一个框长在弹窗里，弹窗自己
  已经浮起来了，再加一层就是浮起来的东西上面又浮了一块，框会从卡片上鼓出来。同理：浮层里的
  分隔线、卡片里的卡片、已经有边框的容器里再套一个边框，都要先问一句「这一层深度是从哪儿来的」。
- **边界只画一次。** 框有了自己的下沿，它和底下那排动作之间就不要再横一条线——同一件事说了
  两遍，看上去是两件。
- **间距读变量，不写字面量。** `--ly-composer-in` / `--ly-composer-x` 这类变量是一套呼吸感，
  抄成 `12px` `16px` 之后就和源头断了，那边一调，这边原地不动。

**搬样式的时候不要顺手改掉功能。** 提交弹窗里那颗语言切换按钮本来挂在「这一次会不会自动生成」
上：打下第一个字它就消失、框跟着矮一截。对着屏幕的人看到的不是条件渲染，是「这个功能被删了」。
一个控件只因为输入框里有没有字就来回出没，本身就是毛病——常驻，顺带让框有了固定的高度。凡是
改版后**少了一样能点的东西**，都要当成回归来对待，哪怕它是既有逻辑。

拿不准两种写法哪个对，就在真窗口里把它们前后摆出来各拍一张，比记忆可靠。
...
**凡是在真窗口里验的，都录下来交给用户**，不要只把一串 ✅ 贴在回复里。视频和截图**都**写到
...
| `packages/contract/` | 渲染进程与主进程之间那条线，218 个方法写在一处 |
