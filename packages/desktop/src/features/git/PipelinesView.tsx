...
function StatusIcon({
	...
	if (status === "in_progress") {
		return <StatusSpinner size={size} className="text-amber-500" />;
	}
...
export function PipelinesView({ cwd, onOpenRelease, toolbar, active = true }: PipelinesViewProps) {
	...
	const controls = <div className="flex shrink-0 items-center gap-1">
		<IconButton size="sm" icon={(inspectRun ? detailLoading : refreshing) ? <ActionSpinner size={13.5} /> : <RefreshCw size={13.5} />}
