...
export function ApprovalOverlay() {
	...
	return <div data-approval-region className={`flex shrink-0 justify-center pb-2 ${compact ? "px-4" : "px-8"}`}>
		<div data-approval-card className="ly-glass flex w-full max-w-[var(--ly-content)] flex-col overflow-hidden rounded-xl border border-line">
			<div className="flex shrink-0 items-center gap-2 px-4 py-2.5">
				...
				<button type="button" aria-expanded={!collapsed} aria-label={translate(collapsed ? "question.expand" : "question.collapse")} onClick={() => setCollapsedId(collapsed ? null : request.id)} className="grid h-7 w-7 shrink-0 place-items-center rounded-md text-ink-muted hover:bg-card-hover"><ChevronDown size={15} className={collapsed ? "rotate-180" : ""} /></button>
			</div>
			<div hidden={collapsed} className="min-h-0">{interactive ? <>
				<p className="px-4 pb-1 text-caption text-ink-muted">{translate("question.fullAccessNote")}</p>
				<QuestionChoices key={request.id} options={request.options ?? []} allowCustomInput={request.allowCustomInput === true} selectionMode={request.selectionMode} allowSkip={request.allowSkip} defaultOptionIndex={request.defaultOptionIndex} answer={decision => respond(request.id, decision, sessionId ?? undefined)}>{context}</QuestionChoices>
			</> : <>
