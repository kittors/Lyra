...
> **2026-09-24：两层容器已经合成一层**（[ADR-0023](../adr/0023-containers-belong-to-sessions.md)）。
> 面板只属于会话，单屏是只有一屏的分屏，窗口本身没有面板。下面的矩阵和冲突清单写于两层并存的
> 时候，现在留作病历：
>
> - C0（窗口层读了一把空钥匙）和 C1（从内容里打开的面板落进窗口层）的前提已经不存在了。
>   `dw:dock:at`、`firstSession` 这些补丁也随之删除。
> - C2、C3、C5 的修法原样保留，钥匙仍是 `dw:panedock:<会话>` 和 `dw:homes`。
> - C4（跨屏拖动面板）仍然没做。
> - 第七节末尾的「已知限制」已修，见那里。
>
> 这一轮暴露出来、而下面没有列到的两种状态，都来自同一个根因：
>
> - 分屏时，原会话的任务面板独占了窗口右侧；
> - 关掉一屏回到单屏时，浏览器嵌在标题栏底下。
>
> 它们在新模型里已经不可能出现。
...
从前：出身的那一屏被关掉之后，在它的面板窗口上点「收回」**什么都不会发生，也不报错**。链路是
`dockBack` 的 `paneLive` 为假 → 返回 false → 主窗口 `if (!dockBack(…)) return;`；而主进程那侧
`requestRestorePanel` 只要主窗口在就返回 `ok: true`，面板窗口拿到的是成功。

现在面板属于会话而不是某一格屏（ADR-0023），「那一屏不在了」不再是死路：收回时原会话不在屏上，
主窗口先把它请回屏上（`provideReveal`），等那一屏量好尺寸再放回面板；三秒内还没请回来（或者那是
一个还没发出的空白对话），就把面板写进那个会话存着的布局，下次打开它时面板就在。`dockBack`
因此只在「这个窗口里根本没有工作区」时失败。真窗口验过：主窗口换到别的会话之后在弹出的终端上
点收回，原会话回到屏上、终端满高在它右边、面板窗口关闭。
