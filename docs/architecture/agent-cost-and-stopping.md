...
| 项目 | 当前行为与决策 | 验证或剩余项 |
| --- | --- | --- |
| A1 结果老化 | `AgedToolPruner` 在请求组装前每 20 次批量检查，年龄至少 20 个助手回合，复用 `worthPruning` 保护长缓存后缀；会话内保持同一裁剪视图，原日志不变，skill 不裁 | 单测覆盖边界、缓存、跨用户回合和真实请求接线。生产策略历史重放省 8.04%，不是原先无缓存约束的 52.6% 理想值 |
| A2 grep | rg 与 fallback 共用单行 2,000、总行结果 12,000 UTF-16 字符闸门，不拆 surrogate；rg 增量读行，details 同步受限 | 巨型单行两条路径 red/green；历史最大结果从 12,853,666 降至 11,999 字符 |
...
- `pnpm audit:prune`：275 份日志（含空会话）、27,132 个请求；原携带 25,764,303,416 字符·请求，
  单独老化后 23,694,825,833，单独 grep 后 20,144,012,655，合并后 18,219,632,547（下降 **29.28%**）。
  这是生产裁剪策略的工具文本重放，未模拟模型行为、压缩、缓存命中或账单，不能替代原目标的真实验收。
...
**仍未通过的原目标**：总携带成本下降 ≥50%、最贵结果 ≤20M token·轮的完整生产口径验证、
复杂任务平均 ≥2.0 调用/轮、修复后真实缓存与完成质量不恶化。需要新任务样本；不降低目标、不改写历史、
不以更激进的停机或裁剪来凑数。旧文中的“裁剪省钱约 25%”与“15 分钟变 3 分钟”均未证实。
